...
export default function BlastReportForm() {
  // State Management
  const [reportId, setReportId] = useState("BLT-2025-001");
  const [blastId, setBlastId] = useState("");
  const [dateTime, setDateTime] = useState("");
  const [blastLocation, setBlastLocation] = useState("");
  const [rockType, setRockType] = useState("");
  const [groundStability, setGroundStability] = useState("");
  const [waterPresence, setWaterPresence] = useState("");
  ...
  const [holeCondition, setHoleCondition] = useState<string[]>([]);
  const [safetyChecklist, setSafetyChecklist] = useState<string[]>([]);

  // Handle Checkbox Toggles
  const toggleHoleCondition = (value: string) => {
    setHoleCondition((prev) =>
      prev.includes(value) ? prev.filter((v) => v !== value) : [...prev, value]
    );
  };

  const toggleSafetyChecklist = (value: string) => {
  ...
  const handleSaveDraft = () => {
    console.log("Saving Draft...", getFormData());
  };

  const handleSubmit = () => {
    console.log("Submitting Report...", getFormData());
  };
